...
public class Real_time_PerfMonitor_vm {
	...
	public static void main(String[] args) 
	{
		...
		try {
			...
			//HostSystem host = (HostSystem) new InventoryNavigator(si.getRootFolder()).searchManagedEntity("HostSystem", VMNAME);
	      ...
	      /*  HashSet<Integer> hashedArray = new HashSet<Integer>( );
			...
			for (int i = 0; pembs != null && i < pembs.length; i++) 
			{
				...
				for(int z=0;z<array_ids_vm.length;z++)
				 {
					...
					if(vals[j].getId().getCounterId()==array_ids_vm[z])
					{
				...
				try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(vmlogpath, true)))) {
				    out.println(log);
				}catch (IOException e) {
